...
MemDict, MemArr := LoadSettings(user_settings_path, "Members")
...
Gui, Submit, NoHide
memArr := [Ais, Cam]
MemDict := PairMembers(MemDict, stoArray)
...
toggleSyn(memDict, tog)
{
	...
	if(tog=0){
		for x, y in memDict{
			if(memDict[x] != 0){
				total := splashStatus[1]+splashStatus[2]+splashStatus[3]+splashStatus[4]
...
LoadSettings(path, sect)
{
	...
	For x, y in memSection
	{
		...
		Member_Array[x] := temp[2]
	}
	return Member_Settings, Member_Array
}
